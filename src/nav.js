...
export function BottomNav({ route, setRoute }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingTop: 8, paddingBottom: 10, paddingHorizontal: 6, backgroundColor: C.railBg, borderTopWidth: 3, borderTopColor: C.paperDeep }}>
      {NAV.map((n) => {
        const on = route === n.key;
        return (
          <Pressable key={n.key} onPress={() => setRoute(n.key)} accessibilityRole="button" accessibilityLabel={n.label} accessibilityState={{ selected: on }} style={{ alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 5, paddingHorizontal: 4, minHeight: 48, flex: 1 }}>
  ...
  return (
    <View style={{ width: 240, paddingVertical: 22, paddingHorizontal: 14, backgroundColor: C.railBg, borderRightWidth: 3, borderRightColor: C.paperDeep }}>
      ...
      {NAV.map((n) => {
        const on = route === n.key;
        return (
          <Pressable key={n.key} onPress={() => setRoute(n.key)} accessibilityRole="button" accessibilityLabel={n.label} accessibilityState={{ selected: on }} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 11, paddingHorizontal: 12, marginBottom: 4, borderRadius: 6, minHeight: 44, backgroundColor: on ? 'rgba(255,209,128,0.16)' : 'transparent' }}>
