...
export function BottomNav({ route, setRoute }) {
  return (
    <ImageBackground source={KIT.stone} resizeMode="repeat" style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingTop: 10, paddingBottom: 12, paddingHorizontal: 8, borderTopWidth: 3, borderTopColor: C.stoneLine, boxShadow: 'inset 0px 3px 0px rgba(255,255,255,0.4), inset 0px -3px 7px rgba(0,0,0,0.22), 0px -4px 12px rgba(0,0,0,0.22)' }}>
      {NAV.map((n) => {
...
export function SideRail({ route, setRoute, stage, onSignOut, userName }) {
  return (
    <ImageBackground source={KIT.stone} resizeMode="repeat" style={{ width: 264, paddingVertical: 24, paddingHorizontal: 16, borderRightWidth: 4, borderRightColor: C.stoneLine, boxShadow: 'inset -3px 0px 0px rgba(0,0,0,0.22), inset 3px 0px 0px rgba(255,255,255,0.35), inset 0px 3px 0px rgba(255,255,255,0.3)' }}>
      {/* brand */}
      ...
      <View style={{ padding: 16, borderRadius: 18, backgroundColor: C.paper, borderWidth: 2, borderColor: C.paperDeep, boxShadow: 'inset 0px 2px 0px rgba(255,255,255,0.6)' }}>
        ...
        <View style={{ marginTop: 8 }}>
          <ProgressBar pct={(stage.xp / stage.next) * 100} color="xp" height={16} />
        </View>
      </View>
